...
#define BUZZER_PIO_IDX_MASK (1u << BUZZER_PIO_IDX) // esse já está pronto.
...
int get_startstop(){     //funcao q retorna se o botao ta apertado ou nao
	return pio_get(START_PIO,PIO_INPUT,START_PIO_IDX);
}

int get_selecao(){   //segunda funcao para o botao 2 onde escolho que musica tocar
	return pio_get(SELECAO_PIO,PIO_INPUT,SELECAO_PIO_IDX);
}

void buzzer_test(int freq){   //gera no pino do buzzer uma funcao de onda quadrada de frequencia definida
	int t;
	t=1E6/freq; //transformando pra microsegundos
...
void tone(int freq, int time){
	float pulsos;
	pulsos = (float) freq * (float) time/1000;
